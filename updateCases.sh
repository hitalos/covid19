...
lastMonths="2-03|2-02|2-01|1-12|1-11|1-10|1-09|1-08"
...
awk "BEGIN { FS=\",\"; OFS=\",\" } /202($lastMonths).*state/ { print \$1,\$2,\$5,\$6,\$8,\$10,\$11 }" "$TEMP" >> "$REPO/casos.csv"
gzip -9 -k -f "$REPO/casos.csv"

for UF in $STATES; do
	echo "Filtrando dados de $UF..."
	echo "date,state,city,confirmed,deaths,is_last" > "$REPO/$UF/casos.csv"
	awk "BEGIN { FS=\",\"; OFS=\",\" } /202($lastMonths).*,$UF,.*,city/ { print \$1,\$2,\$3,\$5,\$6,\$8 }" "$TEMP" >> "$REPO/$UF/casos.csv"
